...
describe('HttpTranslitRuleLoader', () => {
    ...
    it('should be created', () => {
        const httpTranslitRuleLoader = TestBed.get<HttpTranslitRuleLoader>(HttpTranslitRuleLoader);
...
describe('HttpTranslitRuleLoader#load', () => {
    ...
    it('should return translit rule response',
        inject([HttpTestingController], (httpMock: HttpTestingController) => {
            ...
            const httpTranslitRuleLoader = getTestBed().get<HttpTranslitRuleLoader>(HttpTranslitRuleLoader) as HttpTranslitRuleLoader;
            const ruleName = 'mock-rule';

            httpTranslitRuleLoader.load(ruleName).subscribe((rule: TranslitRule) => {
                expect(rule).toEqual(mockRule);
                expect(rule.description).toEqual('mock rule');
                expect(rule.version).toEqual('1');
                expect(rule.phases.length).toBe(2);
            });
    ...
    it('should throw an error message when loader returns an error',
        inject([HttpTestingController], (httpMock: HttpTestingController) => {
            const mockRule: TranslitRule = {
                description: 'Error!',
                phases: [{
